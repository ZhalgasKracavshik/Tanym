/**
 * Проверка подключения к Google Gemini.
 *
 * Запуск:  node scripts/check-ai.mjs
 *
 * Скрипт читает GEMINI_API_KEY из .env.local и делает один реальный запрос.
 * Нужен, чтобы отделить проблемы с ключом от проблем в коде приложения:
 * если здесь всё зелёное, значит ключ, сеть и модель в порядке.
 */
