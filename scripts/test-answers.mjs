/**
 * Проверка распознавания ответов в архиве.
 *
 * Запуск:  npm run test:answers
 *
 * Каждый случай здесь взят из реального поведения, а не придуман: главный —
 * пересказ условия («там сказано, что скорость 20 м/с» в задаче, где ответ
 * тоже 20). Наивная проверка засчитывала его как решение и выдавала ученику
 * разбор ни за что.
 */
...
const CASES = [
  ...
  // Ответ 48 в условии не встречается — маркер не нужен.
  ...
  // Односимвольные ответы: цифра легко попадается в обычной речи, и наивная
  // проверка засчитывала решение по фразе, не имеющей отношения к задаче.
