...
export function useEffectiveProfile(): Profile | null {
  ...
  return useMemo(() => {
    ...
    /*
      Ни админ, ни внешний центр сюда не приводятся: в учебных типах роли
      всего две. Админ — не «ученик с правами», а отдельная роль без
      класса, предметов и плана; центр вообще не участник школы. Их
      страницы другие, кабинет и план им показывать нечем.
    */
    if (schoolProfile.role === 'admin' || schoolProfile.role === 'center') return local;
