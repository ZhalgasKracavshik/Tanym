 ...
 *
 * Здесь только 'achievement_post': в achievement-photos лежит снимок, который
 * ученик выложил сам и осознанно. У 'achievement_approved' вложение — скан
 * грамоты из achievement-proofs, а на нём почти всегда напечатаны имя ребёнка,
 * класс и школа. Развернуть такой документ во всю ширину общешкольной ленты
 * значит опубликовать персональные данные за ученика и без его участия, поэтому
 * подтверждение сюда не попадает вовсе: оно остаётся в портфолио, доступное по
 * ссылке «Диплом» — то есть по отдельному действию. То же правило и теми же
 * словами продублировано в ActivityFeed.tsx: лент две, и разойтись им нельзя.
 *
 * Побочно это снимает и вторую беду: подтверждение часто прикладывают в PDF, а
 * <img> на PDF отрисовывался значком битой картинки во всю ширину карточки.
 */
