  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <h2 className="text-lg font-medium leading-snug [overflow-wrap:anywhere] text-ink-900">{headline}</h2>

        {meta && <p className="mt-1.5 [overflow-wrap:anywhere] text-sm text-ink-400">{meta}</p>}

        {/*
          overflow-wrap: anywhere, а не break-words — разница
          принципиальная.

          Объявления пишут люди, и в базе уже лежит описание, где текст
          слипся в одно слово длиной в строку: «Отсутствиеаналитики.
          Нетобратнойсвязи.Студентытратятвремявпустую».

          break-words позволяет ПЕРЕНЕСТИ такое слово, но не уменьшает его
          вклад в минимально возможную ширину блока. У элемента сетки
          min-width равен auto, то есть он растягивается до этой самой
          минимальной ширины — и вся сетка «Возможностей» на телефоне
          разъезжалась до 919 пикселей при экране в 375. Замер: минимальная
          ширина описания 861 пиксель.

          anywhere этот вклад уменьшает, а min-w-0 на ссылке-обёртке
          снимает растягивание у самого элемента сетки. Нужно и то, и
          другое: первое чинит текст, второе — любое будущее содержимое.
        ...
        */}
        <p
          className="mt-3 mb-4 [overflow-wrap:anywhere] text-sm leading-relaxed text-ink-600"
          style={
            clampLines > 0
              ? {
                  display: '-webkit-box',
